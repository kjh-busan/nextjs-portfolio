...
export default function ProjectItem({ data }) {
  ...
  const relationship =
    data.properties.relationship.checkbox == true ? "좋음" : "관계회복 노력중";
  ...
  const calculatedPeriod = (lifeperiodStart, lifeperiodEnd) => {
    ...
    const endDateStringArray =
      lifeperiodEnd == "현재" ? endToday : lifeperiodEnd.split("-");
    ...
    <div className="project-card">
      ...
      <div className="p-4 flex flex-col">
        ...
        <h4 className="mt-4 text-xl">
          {lifeperiodStart} ~ {lifeperiodEnd} (만
          {calculatedPeriod(lifeperiodStart, lifeperiodEnd)}세)
        </h4>
